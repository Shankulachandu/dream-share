...
function Stories() {
  ...
  const markViewed = async (storyId) => {
    if (!myId) return;
    await axios.post(`${API_URL}/story/view`, {
      story_id: storyId, viewer_id: parseInt(myId)
    });
  };
        ...
        <div style={styles.viewer}>
          ...
          <div style={styles.viewerHeader}>
            <div style={styles.viewerUser}>
              <div style={styles.viewerAvatar}>{selectedUser.username[0].toUpperCase()}</div>
              <div>
                <p style={styles.viewerUsername}>@{selectedUser.username}</p>
                <p style={styles.viewerTime}>{new Date(currentStory.created_at).toLocaleTimeString()}</p>
              </div>
            </div>
...
const styles = {
  storiesBar: { display: 'flex', gap: '16px', padding: '16px', overflowX: 'auto', background: 'rgba(255,255,255,0.03)', borderRadius: '16px', marginBottom: '24px', scrollbarWidth: 'none' },
  storyItem: { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px', cursor: 'pointer', flexShrink: 0 },
  addStoryBtn: { cursor: 'pointer' },
  addStoryCircle: { width: '64px', height: '64px', borderRadius: '50%', background: 'rgba(108,99,255,0.2)', border: '2px dashed rgba(108,99,255,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center' },
  plusIcon: { fontSize: '28px', color: '#a78bfa', fontWeight: '300' },
  storyRing: { width: '68px', height: '68px', borderRadius: '50%', padding: '2.5px', display: 'flex', alignItems: 'center', justifyContent: 'center' },
  storyAvatarInner: { width: '60px', height: '60px', borderRadius: '50%', background: 'linear-gradient(135deg, #1a1a2e, #2d2d4e)', border: '2px solid #0f0f1a', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: '700', fontSize: '22px' },
  storyUsername: { fontSize: '11px', color: '#a0a0b0', textAlign: 'center', maxWidth: '68px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' },
  ...
  modalCard: { background: '#1a1a2e', border: '1px solid rgba(108,99,255,0.3)', borderRadius: '20px', padding: '28px', width: '90%', maxWidth: '420px' },
  modalTitle: { color: 'white', fontSize: '18px', fontWeight: '700', marginBottom: '16px', textAlign: 'center' },
  uploadPreview: { borderRadius: '12px', overflow: 'hidden', marginBottom: '16px' },
  previewMedia: { width: '100%', maxHeight: '300px', objectFit: 'cover', display: 'block', borderRadius: '12px' },
  captionInput: { width: '100%', padding: '12px 16px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.05)', color: 'white', fontSize: '14px', outline: 'none', marginBottom: '16px' },
  ...
  uploadBtn: { flex: 1, padding: '12px', background: 'linear-gradient(135deg, #6c63ff, #a78bfa)', color: 'white', border: 'none', borderRadius: '12px', cursor: 'pointer', fontSize: '14px', fontWeight: '600' },
  viewer: { position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'black', zIndex: 2000, display: 'flex', flexDirection: 'column' },
  progressBars: { display: 'flex', gap: '4px', padding: '12px 12px 0', position: 'absolute', top: 0, left: 0, right: 0, zIndex: 10 },
  progressBar: { flex: 1, height: '3px', background: 'rgba(255,255,255,0.3)', borderRadius: '2px', overflow: 'hidden' },
  progressFill: { height: '100%', background: 'white', borderRadius: '2px', transition: 'width 0.1s linear' },
  viewerHeader: { position: 'absolute', top: '20px', left: 0, right: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0 16px', zIndex: 10 },
  viewerUser: { display: 'flex', alignItems: 'center', gap: '10px' },
  ...
  captionText: { color: 'white', fontSize: '16px', fontWeight: '500', textShadow: '0 2px 8px rgba(0,0,0,0.8)', background: 'rgba(0,0,0,0.4)', padding: '8px 16px', borderRadius: '20px', display: 'inline-block' },
  tapLeft: { position: 'absolute', top: '60px', left: 0, width: '35%', bottom: '80px', zIndex: 9, cursor: 'pointer' },
  tapRight: { position: 'absolute', top: '60px', right: 0, width: '35%', bottom: '80px', zIndex: 9, cursor: 'pointer' }
};
